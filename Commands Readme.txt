...
(ContactManagementApp /sqlcmd -S localhost -U sa -P pass@word1)
...
*	Steps to Apply Migration(Code first approach):
	...
	- Run following command to apply migration-
             (ContactManagementApp /dotnet-ef database update)
...
* To run the test cases in CMD, Run the following command to test the application:
	(ContactManagementApp.Tests/dotnet test --logger "console;verbosity=detailed")
